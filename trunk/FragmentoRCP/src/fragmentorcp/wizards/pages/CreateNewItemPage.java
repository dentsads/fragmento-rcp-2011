...
public class CreateNewItemPage extends GuiModelPropertyChange_IWizardPage {
	...
	/** The btn browse for local. */
	private Button btnBrowseForLocal;

	/** The btn browse. */
	private Button btnBrowse;

	/** The file filter path. */
	...
	/**
	 * Instantiates a new creates the new item page. Title and Description of
	 * the page are set. The presenter is loaded and the view and model is
	 * added.
	 * 
	 * @param pageName
	 *            the page name
	...
	/**
	 * Instantiates a new creates the new item page. This constructor is called
	 * when an existing relation is to be updated.
	 * 
	 * @param pageName
	 *            the page name
	 * @param relation
	...
	public void createControl(final Composite parent) {
		...
		combo.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				presenter.setModelProperty("combo", combo.getSelection()
						.toString());
			}
		});
		combo.setItems(new String[] { "Annotation", "Container",
		...
		btnBrowseForLocal.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (btnBrowseForLocal.getSelection()) {
					presenter.setModelProperty("localBrowseButtonSelected",
							true);
				} else {
					presenter.setModelProperty("localBrowseButtonSelected",
							false);
		...
		btnBrowse.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				fileDialog.setFilterExtensions(new String[] { "*.xml",
						"*.wsdl", "*.bpel*", "*.*" });
				fileDialog.setFilterNames(new String[] { "XML", "WSDL", "BPEL",
						"Any" });
		...
		text_2 = new Text(grpContent, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL
		...
		combo_1.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				presenter.setModelProperty("combo_1", combo_1.getSelection()
						.toString());
			}
		});
		combo_1.setItems(new String[] { "annotation", "container",
				"deployment", "modeller", "transformation", "wsdl" });
		combo_1.setBounds(100, 25, 194, 27);
		if (updateRelation) {
			combo_1.setText(this.presenter.getOperator().relationAdapter(
		...
		text_3.setItems((String[]) this.presenter.getOperator()
				.getArtefactList().toArray(new String[0]));
		...
		text_4.setItems((String[]) this.presenter.getOperator()
				.getArtefactList().toArray(new String[0]));
		 ...
		 * depending on the called constructor (if it is an update relation
		 * call).
		 */
		if (updateRelation) {
		...
		if (event.getPropertyName().equals("localBrowseButtonSelected")) {
			...
			if ((Boolean) event.getNewValue()) {
				...
				if (text_1.isEnabled()) {
					...
					this.presenter.getOperator().createNewArtefact(
							combo.getText(), text.getText(), content);
				} else {
					this.presenter.getOperator().createNewArtefact(
							combo.getText(), text.getText(), text_2.getText());
				}
				this.btnNewButton.setEnabled(false);
				this.setPageComplete(true);
